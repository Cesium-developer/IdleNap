...
可以指定一个时间段，让 AutoSleep 只在这个时间段内允许触发（例如凌晨 2:00 到 7:00）。超出时间窗口时，计时器会自动重置，电脑不会进入睡眠。
...
可以直接编辑 `C:\ProgramData\AutoSleep\settings.json`，调整以下参数：
